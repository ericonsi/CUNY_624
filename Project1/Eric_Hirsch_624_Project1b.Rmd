...
tsKWH <- read_excel("D:\\RStudio\\CUNY_624\\Project1\\ForecastLoad2.xlsx") |>
    mutate(Month=yearmonth(Month2)) |>
    as_tsibble(index=Month)
...
dfForecast$Month <- str_replace(dfForecast$Month, "-", " ")
...
```{r, fig.height=2}
trainKWH <- Summarize_KWH(tsForecast5, "ATM1")
```
...
The ARIMA model has poor residuals - it is possible there would need to be more differencing to make the series stationary. ETS_AAA has the second lowest RMSE and the residuals look like white noise, so we'll proceed with that. 

The model estimates 92,108,102 KW for 2014.  
...
fcKWH %>%
  ...
  geom_line(aes(y = .fitted), col="#D55E00",
            data = augment(fitKWH)) +
  labs(y="Count", title="KWH - ES(A-A-A))") +
  guides(colour = "none")
