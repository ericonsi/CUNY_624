...
x43 <- filter(tsATM1, row_number() == 43)
x45 <- filter(tsATM1, row_number() == 45)
Cash44 = (x43$Cash+x45$Cash)/2

x46 <- filter(tsATM1, row_number() == 46)
x48 <- filter(tsATM1, row_number() == 48)
Cash47 = (x46$Cash+x48$Cash)/2

x52 <- filter(tsATM1, row_number() == 52)
x54 <- filter(tsATM1, row_number() == 54)
Cash53 = (x52$Cash+x54$Cash)/2

tsATM1a <- tsATM1 %>%
  mutate(Cash=ifelse(row_number() == 44, Cash44, Cash)) %>%
    mutate(Cash=ifelse(row_number() == 47, Cash47, Cash)) %>%
    mutate(Cash=ifelse(row_number() == 53, Cash53, Cash))

```
```{r}

tsATM1b <- tsATM1a %>%
  group_by(Year=year(xDate), Month=month(xDate)) %>%
  dplyr::summarize(MonthlyCash=sum(Cash)) %>%
  mutate(xDate=paste(Year,"-",Month))
...
The RMSE for the seasonalally adjusted data is 1.55 - between the other two.  It is not lower, and therefore we reject it.
