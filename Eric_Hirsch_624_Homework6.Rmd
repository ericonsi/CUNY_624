...
fc2 %>% autoplot(aus_airpassengers) + labs(labs(title = "ARIMA 0-1-0")) 
...
```{r}
fit2 <- aus_airpassengers %>%
  model(ARIMA(Passengers ~ pdq(2,1,2), include.mean=FALSE))
...
tsUSA <- global_economy %>%
  filter(Country=="United States")

tsUSA <- tsUSA |>  mutate(GDP=box_cox(GDP, Get_lambda(tsUSA, "GDP")))
...
fc2 %>% autoplot(tsUSA) + labs(labs(title = "ARIMA 1-1-0")) 
...
fc2 %>% autoplot(tsUSA) + labs(labs(title = "ARIMA 1-1-1")) 

fit2 <- tsUSA %>%
  model(ARIMA(GDP ~ pdq(1,2,0)))
...
fc2 %>% autoplot(tsUSA) + labs(labs(title = "ARIMA 1-2-0")) 
...
fit2 <- tsU  %>% model(ETS(GDP ~ error("M") + trend("A") + season("N")))
...
fc2 %>% autoplot(tsU) + labs(labs(title = "ETS MAN")) 
